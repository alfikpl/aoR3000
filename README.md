...
The aoR3000 soft processor core has the folowing features:
- 5 stage pipeline (single-issue, in-order, forwarding, hazard detection);
- implements all MIPS I instructions (this includes the priviledged coprocessor 0 instructions);
...
- multiplication implemented in hardware using vendor-independent modules (Altera Quartus II infers embedded mulipliers);
...
The tester simulates the folowing hardware devices:
...
To compile the SoC the folowing steps have to be taken:
...
- verify that the folowing text is displayed in the terminal: 'Press any key to boot kernel...';
